...
function getSteps() {
  return ["General information", "Detailed information", "Questions", "Video"];
}
...
class HorizontalNonLinearStepper extends React.Component<Props, State> {
  ...
  render() {
    const { classes, userStep, userIsSubmited } = this.props;
    ...
    return (
      <div className={classes.root}>
        <Stepper
          activeStep={userIsSubmited ? thisStep(userStep) : 8}
          connector={connector}
        >
          {steps.map(label => (
            <Step key={label}>
